const url = 'https://adventure-time-quote-api.glitch.me/api/random';
...
function displayResults(quote){
    ...
    let heading = document.createElement("h1");  //creates a tag
    let img = document.createElement("img");

    section.appendChild(heading);  //append that info to the document with that tag
    section.appendChild(img);

    heading.textContent = quote.split(":")[1];  //to grab the second element in the array

    let character = quote.split(":")[0];  //to grab the first element in the array

}

// OR --- heading.textContent = quote  //then tell it what text to append to the tag

    //or can use a .split() method - 
    //heading.textContent = quote.split(":")
